...
for subject in SUBJECTS:
    ...
    compare_ses_list = COMPARE_SESSIONS.get(subject, [])
    colors = {ses: COMPARE_PALETTE[i % len(COMPARE_PALETTE)]
              for i, ses in enumerate(compare_ses_list)}
    ...
    print(f"  Compare sessions: {compare_ses_list}")
    print(f"{'=' * 65}")
    ...
    print(f"\n{'=' * 75}")
    print(f"  RESTING-STATE COMPARISON  {subject}  "
          f"(reference n={n_ref} sessions)")
    print(f"{'=' * 75}")
    ...
    for ses in compare_ses_list:
        ...
        path_s = out_dir / f"{subject}_resting_state_ses{ses}_strip.png"
        ...
        darker = {"#E53935": "#B71C1C", "#1565C0": "#0D47A1",
                  "#F9A825": "#E65100", "#AB47BC": "#6A1B9A", "#FF7043": "#BF360C"}
        bar_colors = [darker.get(color, color) if abs(z) >= 2 else color
                      for z in z_scores]
        ...
        ax_z.set_title(
            ...
            f"(Reference = all sessions except {compare_ses_list})",
        ...
        path_z = out_dir / f"{subject}_resting_state_ses{ses}_zscores.png"
    ...
    out_csv = out_dir / f"{subject}_resting_state_features.csv"
    with open(out_csv, "w", newline="") as fh:
        ...
        for r in records:
            ...
            group     = f"comparison_{ses_num}" if ses_num in compare_ses_list else "reference"
